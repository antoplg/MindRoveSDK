  ...
  if(Timer_01.Tick()){ 
    ...
    // --- EMG - ONE MODULE + GRIPPER CONTROL ---
      ...
      if (Serial.available() >= 8 ) { // Ensure enough bytes are available
        ...
        // Invio delle variabili modificate indietro a python
        if (CALIBRATION == true){
          flex  = 1;
          ext   = 1;
        } else {
          flex  = bytesToFloat(buffer, 0);
          ext   = bytesToFloat(buffer, 4);
        }
      }
      ...
      if (presentLoad > 160 && right_sock_scaled < -0.5) gripperVel   = 0;
      else if (flex > 0.5 && ext <0.5) gripperVel                     = 1;
      else if (ext > 0.5 && flex <0.5) gripperVel                     = -1;
      else gripperVel                                                 = 0;
