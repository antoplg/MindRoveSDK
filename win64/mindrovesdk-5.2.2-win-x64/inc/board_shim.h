...
public:
    ...
    static int get_package_num_channel (
        int board_id, int preset = (int)MindRovePresets::DEFAULT_PRESET);
    /**
    ...
    static std::vector<int> get_ppg_channels (
        int board_id, int preset = (int)MindRovePresets::DEFAULT_PRESET);
    /**
     * get row indices which hold PPG raw data
     * @param board_id board id of your device
     * @throw MindRoveException If this board has no such data exit code is UNSUPPORTED_BOARD_ERROR
     */
    static std::vector<int> get_ppg_raw_channels (
        int board_id, int preset = (int)MindRovePresets::DEFAULT_PRESET);
    /**
     ...
     */
    void add_streamer (
        std::string streamer_params, int preset = (int)MindRovePresets::DEFAULT_PRESET);
    /**
     * delete streamer
     * @param streamer_params use it to pass data packages further or store them directly during
     streaming, supported values: "file://%file_name%:w", "file://%file_name%:a",
    ...
    MindRoveArray<double, 2> get_current_board_data (
        int num_samples, int preset = (int)MindRovePresets::DEFAULT_PRESET);
